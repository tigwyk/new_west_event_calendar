...
import { eventService, commentService, rsvpService, userService, subscriptions } from '../lib/database'
import { EventRow, CommentRow, RSVPRow } from '../lib/supabase'
...
export const useEvents = () => {
  ...
  const addComment = useCallback(async (eventId: string, text: string) => {
    ...
    // Update the event in state with new comment
    setEvents(prevEvents =>
      prevEvents.map(event => {
        if (event.id === eventId) {
          return {
